...
def build_cgcnn_graph(structure: Structure, 
                      ...
                      max_radius=6.0):
    """
    Construct a CGCNN-style crystal graph and return:
    - node_fea: [N], i.e., atomic numbers (integer type)
    - edge_index: [2, E], i.e., edge connectivity
    - edge_fea: [E, edge_dim], i.e., Gaussian-expanded distances
    """
    # 1. Node features: atomic numbers (int)
    ...
    # node_fea is a vector of atomic numbers with shape [N] and type int64
...
class CGCNNStructureDataset(Dataset):
    def __init__(self, 
        ...
        # calculate extra_fea's mean/std
        self.extra_mean = []
        self.extra_std = []
    ...
    def __getitem__(self, idx):
        ...
        # Construct graph (atomic number -> [N], int64)
        ...
        # Target value (predict a single value only, without normalization)
        y_val = torch.tensor([row[self.target_col]], dtype=torch.float32)  # shape [1]

        # Standardize extra_fea
        extra_fea_vals = []
        ...
        # crystal_atom_idx
        N = node_fea.shape[0]
        crystal_atom_idx = torch.zeros(N, dtype=torch.long)  # All belong to the same crystal
...
class AtomEmbedding(nn.Module):
    """
    (atomic_number) => embedding vector
    """
...
def collate_cgcnn(batch_list):
    """
    Concatenate a list of individual samples into a batched graph for a single forward pass through the model.
...
def predict_test_set(model, test_loader, target_col, output_csv="test_predictions.csv"):
    """
    Evaluate test set performance for a single-output model predicting melting points.
    The model is expected to output log(melting_point + 1), which must be converted back using exp() - 1.
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch_data in test_loader:
            ...
            predictions.extend(pred.squeeze(1).cpu().tolist())  # Convert to [B]
            true_values.extend(true.squeeze(1).cpu().tolist())  # Convert to [B]
    ...
    print(f"[FINISHED] Prediction results saved to: {output_csv}")
...
def filter_by_element_overlap(df_all, allowed_elements, threshold=0.5):
    def is_allowed_fraction_ok(row_elements):
        try:
            # String, convert it into a real list first
            if isinstance(row_elements, str):
                row_elements = ast.literal_eval(row_elements)
        except Exception:
            return False  # Filter out entries if parsing fails
...
def main_transfer_training_example(
    ...
    model.to(device)
    print(f"[LOAD] Loaded pretrained CGCNN model from {pretrained_model_path}")

    # Optionally freeze partial parameters (fine-tune only the output layer)
    if freeze_partially:
        for name, param in model.named_parameters():
            param.requires_grad = False
        for param in model.fc_out.parameters():
            param.requires_grad = True
        print("[FREEZE] Froze all layers except fc_out (only finetuning output layer).")
    ...
    for epoch in range(1, EPOCH_NUM + 1):
        ...
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), BEST_MODEL_PATH)
            print(f"[SAVED] Best finetuned model saved at epoch {epoch} (val_loss = {val_loss:.4f})")
            early_stop_counter = 0
        else:
            early_stop_counter += 1
            print(f"No improvement. ⏸ Early stop counter: {early_stop_counter}/{PATIENCE}")
            if early_stop_counter >= PATIENCE:
                print("[STOPPING] Early stopping triggered.")
                break
...
def main():
    ...
    print(f"{len(df_allowed)} entries remaining after removing NaNs")
    # Parse JSON and convert to pymatgen.Structure
    ...
    print(f"Number of data entries satisfying Threshold > 0.5: {len(df_filt)}")
    ...
    BEST_MODEL_PATH = f"best_modelB_{ABREV}_all.pth"
    TEST_PREDICTIONS_CSV = f"modelB_test_predictions_{ABREV}_all.csv"
    ...
    BEST_MODEL_PATH_EX = f"best_modelB_{ABREV}_all_extras.pth"
    TEST_PREDICTIONS_CSV_EX = f"modelB_test_predictions_{ABREV}_all_extras.csv"
    ...
    plot_loss_curve(train_losses, val_losses, f"modelB_{ABREV}_loss.png")
    plot_loss_curve(train_losses_ex, val_losses_ex, f"modelB_{ABREV}_loss_extras.png")
    ...
    BEST_FINETUINED_MODEL_PATH = f"best_modelB_{ABREV}_finetuned.pth"
    TEST_PREDICTIONS_CSV_FINETUINED = f"modelB_test_predictions_{ABREV}_finetuned.csv"
    ...
    BEST_FINETUINED_MODEL_PATH_EX = f"best_modelB_{ABREV}_finetuned_extras.pth"
    TEST_PREDICTIONS_CSV_FINETUINED_EX = f"modelB_test_predictions_{ABREV}_finetuned_extras.csv"
    ...
    plot_loss_curve(train_losses, val_losses, f"modelB_{ABREV}_finetuned_loss.png")
    plot_loss_curve(train_losses_ex, val_losses_ex, f"modelB_{ABREV}_finetuned_loss_extras.png")
