...
def train_encoder_decoder(
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_data in dataloader:
            ...
            # forward
            lattice, lattice_norm, coords, types_logits, prop_out = model(
                node_fea, edge_index, edge_fea, crystal_atom_idx, extra_fea
            )
...
if __name__ == "__main__":
    ...
    # Select mode
    parser.add_argument('--mode', type=str, choices=['pretrain', 'finetune', 'generate'], required=True,
                        help="Mode: 'train' to train models, 'generate' to sample new structures")
    
    # For pretrain
    parser.add_argument('--pretrain_data_csv', type=str, default='data_csv/data_T0.5_MP2K_RHO8.csv',
    ...
    # For finetune
    parser.add_argument('--finetune_data_csv', type=str, default='data_csv/data_T0.5_MP2K_RHO8.csv',
                        help="Path to the input data CSV file")
    parser.add_argument('--finetune_epochs', type=int, default=300, help="Number of epochs for finetune encoder-decoder model training")
    parser.add_argument('--finetune_batch_size', type=int, default=32, help="Batch size for finetune encoder-decoder model training")
    parser.add_argument('--save_finetune_endecoder', type=str, default='endecoder_finetune_model.pt', help="Path of fine-tune encoder-decoder model weights")
    parser.add_argument('--save_finetune_diffusion_model', type=str, default='diffusion_finetune_model.pt', help="Path of fine-tune diffusion model weights to load for generation")
    parser.add_argument('--finetune_diffusion_epochs', type=int, default=300, help="Number of epochs for diffusion model finetuning")
    parser.add_argument('--finetune_diffusion_batch_size', type=int, default=32, help="Batch size for diffusion model finetuning")


    # For generate
    parser.add_argument('--generate_data_csv', type=str, default='data_csv/data_T0.5_MP2K_RHO8.csv',
                        help="Path to the input data CSV file")
    parser.add_argument('--load_finetune_diffusion_model', type=str, default='diffusion_finetune_model.pt', help="Path of fine-tune diffusion model weights to load for generation")
    parser.add_argument('--load_finetune_endecoder', type=str, default='endecoder_finetune_model.pt', help="Path of fine-tune encoder-decoder model weights to load for generation")
    parser.add_argument('--num_samples', type=int, default=100, help="Number of structures to generate")
    ...
    if mode == 'pretrain':
        ...
        # Load and preprocess data
        df = pd.read_csv(args.finetune_data_csv)
        df = df.dropna()
        df["structure"] = df["structure"].apply(lambda x: Structure.from_dict(json.loads(x)))
        ...
        # num_atom_types = len(all_atomic_nums) + 1
        num_atom_types = NUM_ATOM_TYPES
        print("Fine-tune z2index:", z2index)
        print("num_atom_types =", num_atom_types)
        ...
        train_dataset = StructureDataset(df_train, cond_cols=cond_cols, extra_feature_cols=extra_feature_cols, max_atoms=MAX_ATOMS, z2index=z2index)
        val_dataset = StructureDataset(df_val, cond_cols=cond_cols, extra_feature_cols=extra_feature_cols, max_atoms=MAX_ATOMS, z2index=z2index)
        ...
        cond_dim = len(cond_cols)
        # Determine max_atoms in dataset and number of atom types (for decoder)
        # max_atoms = int(df["num_of_atoms"].max())
        max_atoms = MAX_ATOMS
        ...
        # === Fine-tune ===
        print("[INFO] Finetuning encoder-decoder model ...")
        endecoder, loss_list = train_encoder_decoder(
            model=endecoder,
            dataset=train_dataset,
            ...
            learning_rate=1e-5,
            device=device,
            lattice_w=20.0, coord_w=10.0, type_w=1.0, prop_w=1.0,
            patience=20,
            save_path=args.save_finetune_endecoder
        )

        # === Plot training losses ===
        plot_training_losses(loss_list, "Finetune-Loss.png")
        ...
        print("Validation results after fine-tuning:")
        for k, v in val_metrics.items():
            print(f"{k}: {v:.4f}")

        print(f"[INFO] Loading pretrained diffusion weights from: {args.load_diffusion_model}")
        state_dict = torch.load(args.load_diffusion_model, map_location=device, weights_only=True)
        endecoder.load_state_dict(state_dict)

        # === Finetuning diffusion model ===
        print("[INFO] Finetuning diffusion model ...")
        diff_model = DiffusionModel(latent_dim=latent_dim, cond_dim=cond_dim, time_embed_dim=32, hidden_dim=128)
    
        diff_model, diff_loss = train_diffusion_model_with_supervision(
            model=diff_model,
            encoder=endecoder.encoder,
            decoder=endecoder.decoder,
            prop_pred=endecoder.prop_pred,
            dataset=train_dataset,
            epochs=args.finetune_diffusion_epochs,
            batch_size=args.finetune_diffusion_batch_size,
            learning_rate=1e-5,
            device=device,
            T=args.t,
            guidance_scale=args.guidance_scale,
            save_path=args.save_finetune_diffusion_model,
            lambda_prop=1.0,
            lambda_lattice=2.0
        )
        ...
        # === Load fine-tuned Encoder-Decoder ===
        print("[INFO] Loading fine-tuned Encoder-Decoder model...")
        endecoder = EncoderDecoderModel(
            atom_fea_dim=64,
            edge_fea_dim=128,
            depth=3,
        ...
        endecoder.load_state_dict(torch.load(args.load_finetune_endecoder, map_location=device, weights_only=True))
